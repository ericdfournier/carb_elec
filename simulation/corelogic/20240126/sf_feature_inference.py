...
invalid_ind = mp['year_built'] > 2024
mp.loc[invalid_ind,'year_built'] = np.nan
...
with tqdm(total = len(classes)) as pbar:
    ...
    for i, c in enumerate(classes):
        ...
        nan_ind = ~mp.loc[:,'year_built'].isna()
        ...
        construction_year = data.loc[:,'year_built']
...
# Save figure to file
fig.savefig('/Users/edf/repos/carb_elec/model/corelogic/20240126/fig/sf_ecdfs.png',
    bbox_inches = 'tight',
    dpi = 300)
...
mp.loc[:,output_cols].to_sql(
    name = 'model_data_sf_inference_20240126',
    con = db_con,
    if_exists = 'replace',
    schema = 'corelogic',
    index = True)
